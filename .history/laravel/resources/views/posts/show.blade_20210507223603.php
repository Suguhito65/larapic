        ...
        <div class="card mt-3">
            ...
            <div class="p-3">
                            ...
                            @can('edit', $comment)
                                <div class="card-footer bg-transparent text-right pt-3">
                                    <form action="{{ route('comments.destroy', ['comment' => $comment->id]) }}" method="post">
                                        @method('DELETE')
                                        {{ csrf_field() }}
                                        <button type="submit" class="delete btn btn-danger btn-sm" style="border-radius: 0.9em">削除</button>
                                    </form>
                                </div>
                            @endcan
                        </div>
                ...
                <div class="text-center mt-3">
                    <a href="{{ route('comments.create', ['post_id' => $post->id]) }}" class="btn btn-primary px-4 mb-1" style="border-radius: 1em">
                        コメント投稿
                    </a>
                </div>
            </div>

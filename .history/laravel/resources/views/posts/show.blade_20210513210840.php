...
<div class="row">
    <div class="col-10 col-md-6 offset-1 offset-md-3">
        ...
        @endif
        <div class="card">
            <div class="card-header text-center text-white" style="background: linear-gradient(45deg, #ffa500, #f00, #ffa500)">ID {{ $post->id }}</div>
            <div class="card-body">
                ...
                <div class="text-center">
                    @if($post->is_liked_by_auth_user())
                        <a href="{{ route('posts.unlike', ['id' => $post->id]) }}" class="btn"><i class="fas fa-heart" style="color: #ee82ee"></i><span class="badge">{{ $post->likes->count() }}</span></a>
                    @else
                        <a href="{{ route('posts.like', ['id' => $post->id]) }}" class="btn"><i class="far fa-heart" style="color: #ee82ee"></i><span class="badge">{{ $post->likes->count() }}</span></a>
                    @endif
                </div>
                @can('edit', $post)

            ...
            <div class="card-body">
                ...
                @can('edit', $post)
                    <div class="row justify-content-center mt-3">
                        <a href="{{ route('posts.edit',['id' => $post->id]) }}" class="btn btn-success mr-3" style="width: 50%">編集</a>
                    
                        <form action="{{ route('posts.destroy', ['id' => $post->id]) }}" method="POST">
                            {{ csrf_field() }}
                            <button type="submit" class="delete btn btn-danger" style="width: 50%">削除</button>
                        </form>
                    </div>
                @endcan
            </div>

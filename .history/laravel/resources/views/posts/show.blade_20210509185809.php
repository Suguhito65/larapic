            ...
            <div class="card-body">
                ...
                @can('edit', $post)
                    <div class="row justify-content-around mt-3">
                        <a href="{{ route('posts.edit',['id' => $post->id]) }}" class="btn btn-success px-4" style="border-radius: 1.2em"><i class="fas fa-pen"></i></a>
                            ...
                            @can('edit', $comment)
                                <div class="card-footer bg-transparent text-right pt-3">
                                    <form action="{{ route('comments.destroy', ['comment' => $comment->id]) }}" method="post">
                                        @method('DELETE')
                                        {{ csrf_field() }}
                                        <button type="submit" class="delete btn btn-danger btn-sm px-3" style="border-radius: 1em">削除</button>
                                    </form>
                                </div>
                            @endcan
                        </div>
